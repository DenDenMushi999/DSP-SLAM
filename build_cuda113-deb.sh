...
if [[ $* == *--install-cuda* ]] ; then
  ...
  if [[ -f cuda-repo-ubuntu2004-11-3-local_11.3.0-465.19.01-1_amd64.deb ]]
  then
    echo "cuda-toolkit installation file already  exists on your filesystem so we won't download it"
  else
...
if [[ $* == *--build-dependencies* ]]; then
  ...
  if [[ ! -d opencv ]]; then
    ...
    echo "It seems opencv repo is already downloaded, so won't download it"
  fi
...
if [[ $* == *--create-conda-env* ]] ; then
  ...
  if { conda env list | grep 'dsp-slam'; } >/dev/null 2>&1; then
    echo "conda environment already exists, so we will recreate it"
    conda env remove -n dsp-slam
  fi

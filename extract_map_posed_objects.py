...
if __name__ == "__main__":
    ...
    with open(os.path.join(map_dir, "MapObjects.txt")) as f:
        ...
        for i in range(N):
            ...
            mesh_tri = trimesh.Trimesh(vertices=mesh.vertices,
                       faces=mesh.faces)

            mesh_tri.apply_transform(pose)
            ...
            # faces = np.concatenate((mesh.faces, np.ones((mesh.faces.shape[0],1))), axis=1)
            # mesh.vertices = (pose.dot(vertices.T)).T[:,:-1]
            # mesh.faces = (pose.dot(faces.T)).T[:,:-1]

            
            # print(mesh.vertices.shape)
            write_mesh_to_ply(mesh_tri.vertices, mesh_tri.faces, os.path.join(save_dir, "%d.ply" % obj_id))
